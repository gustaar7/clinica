...
interface MedicosSectionProps {
  medicos: Medico[]
  search: string
  onAdd: (m: Omit<Medico, 'id'>) => void
  onEdit: (m: Medico) => void
...
interface FormState {
  nome: string; crm: string; email: string; cpf: string; senha: string
}
const emptyForm: FormState = { nome: '', crm: '', email: '', cpf: '', senha: '' }

export default function MedicosSection({
  medicos, search, onAdd, onEdit, onDelete, modalOpen, onModalClose,
}: MedicosSectionProps) {
  ...
  const filtered = medicos.filter((m) => {
    ...
    return m.nome.toLowerCase().includes(q) || m.crm.toLowerCase().includes(q) || m.email.toLowerCase().includes(q)
  })

  const openEdit = (m: Medico) => {
    setEditingId(m.id)
    setForm({ nome: m.nome, crm: m.crm, email: m.email, cpf: m.cpf, senha: '' })
  }

  const handleClose = () => {
    setForm(emptyForm); setEditingId(null); onModalClose()
  }
  ...
  return (
    <>
      <div style={{ border: '0.5px solid #E2E5EB', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{
          padding: '14px 20px', borderBottom: '0.5px solid #E2E5EB',
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr>
              {['', 'Nome', 'CRM', 'E-mail', 'CPF', ''].map((h, i) => (
                <th key={i} style={{
                  padding: '10px 20px', textAlign: 'left', fontSize: 11, fontWeight: 500,
                  color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.04em',
                  borderBottom: '0.5px solid #E2E5EB',
                }}>
                  {h}
                </th>
              ))}
              ...
              <tr>
                <td colSpan={6}>
                  <div style={{ textAlign: 'center', padding: '48px 20px', color: '#9CA3AF' }}>
                    <IconStethoscope size={32} style={{ display: 'block', margin: '0 auto 12px', opacity: 0.4 }} />
                    <span style={{ fontSize: 13 }}>Nenhum médico encontrado</span>
                  </div>
                </td>
              </tr>
            ) : (
      ...
      {isOpen && (
        <Modal
          title={editingId !== null ? 'Editar médico' : 'Novo médico'}
          onClose={handleClose}
          onSave={handleSave}
        >
...
function MedicoRow({ medico, onEdit, onDelete }: { medico: Medico; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <tr
      ...
      style={{ background: hovered ? '#F7F8FA' : '#fff', transition: 'background 0.1s' }}
    >
      ...
      <td style={{ padding: '13px 20px' }}>
        <div style={{ display: 'flex', gap: 4, opacity: hovered ? 1 : 0, transition: 'opacity 0.15s' }}>
          <RowBtn onClick={onEdit} label="Editar"><IconEdit size={14} /></RowBtn>
          <RowBtn onClick={onDelete} label="Excluir" danger><IconTrash size={14} /></RowBtn>
        </div>
      </td>
    </tr>
  )
...
function RowBtn({ onClick, label, danger, children }: { onClick: () => void; label: string; danger?: boolean; children: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      aria-label={label}
      style={{ background: 'none', border: 'none', cursor: 'pointer', color: danger ? '#DC2626' : '#6B7280', display: 'flex', padding: '4px 6px', borderRadius: 6 }}
    >
      {children}
    </button>
  )
