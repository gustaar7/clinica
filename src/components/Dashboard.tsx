...
export default function Dashboard() {
  ...
  // Consultas
  const addConsulta = useCallback((c: Omit<Consulta, 'id'>) => {
    setConsultas((prev) => [...prev, { id: nextId(), ...c }])
    showToast('Consulta agendada')
  }, [])
  const editConsulta = useCallback((c: Consulta) => {
    setConsultas((prev) => prev.map((x) => x.id === c.id ? c : x))
    showToast('Consulta atualizada')
  }, [])
  const deleteConsulta = useCallback((id: number) => {
    setConsultas((prev) => prev.filter((x) => x.id !== id))
    showToast('Consulta removida')
  }, [])

  // Médicos
  const addMedico = useCallback((m: Omit<Medico, 'id'>) => {
    setMedicos((prev) => [...prev, { id: nextId(), ...m }])
    showToast('Médico cadastrado')
  }, [])
  const editMedico = useCallback((m: Medico) => {
    setMedicos((prev) => prev.map((x) => x.id === m.id ? m : x))
    showToast('Médico atualizado')
  }, [])
  const deleteMedico = useCallback((id: number) => {
    setMedicos((prev) => prev.filter((x) => x.id !== id))
    showToast('Médico removido')
  }, [])

  // Pacientes
  const addPaciente = useCallback((p: Omit<Paciente, 'id'>) => {
    setPacientes((prev) => [...prev, { id: nextId(), ...p }])
    showToast('Paciente cadastrado')
  }, [])
  const editPaciente = useCallback((p: Paciente) => {
    setPacientes((prev) => prev.map((x) => x.id === p.id ? p : x))
    showToast('Paciente atualizado')
  }, [])
  const deletePaciente = useCallback((id: number) => {
