...
export default function ConsultasSection({
  consultas, medicos, pacientes, search,
  ...
  const openEdit = (c: Consulta) => {
    setEditingId(c.id)
    setForm({
      pacienteId: String(c.paciente.id),
      medicoId: String(c.medico.id),
      horaData: c.horaData,
      sala: c.sala,
    })
  }
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          { label: 'Total de consultas', value: consultas.length },
      ...
      {isOpen && (
        <Modal
          title={editingId !== null ? 'Editar consulta' : 'Nova consulta'}
          onClose={handleClose}
          onSave={handleSave}
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <SelectField
              ...
              onChange={(v) => setForm((f) => ({ ...f, pacienteId: v }))}
            ...
            <SelectField
              ...
              label="Médico"
              value={form.medicoId}
              onChange={(v) => setForm((f) => ({ ...f, medicoId: v }))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <Field
              ...
              type="datetime-local"
              value={form.horaData}
              onChange={(v) => setForm((f) => ({ ...f, horaData: v }))}
            />
            <Field
              id="f-sala"
              label="Sala"
...
function TimelineRow({
  consulta, status, dotColor, statusLabel, onEdit, onDelete,
}: {
  consulta: Consulta
  status: string
  dotColor: string
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: '#6B7280' }}>
          {consulta.medico.nome} &nbsp;·&nbsp; {consulta.sala}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        >
          {formatDateTime(consulta.horaData)}
        </span>
