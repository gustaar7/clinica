...
export default function PacientesSection({
  pacientes, search, onAdd, onEdit, onDelete, modalOpen, onModalClose,
  ...
  const filtered = pacientes.filter((p) => {
    ...
    return p.nome.toLowerCase().includes(q) || p.email.toLowerCase().includes(q) || p.cpf.includes(q)
  })

  const openEdit = (p: Paciente) => {
    setEditingId(p.id)
    setForm({ nome: p.nome, email: p.email, cpf: p.cpf, senha: '' })
  }
  ...
  return (
    <>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={5}>
                  <div style={{ textAlign: 'center', padding: '48px 20px', color: '#9CA3AF' }}>
                    <IconUsers size={32} style={{ display: 'block', margin: '0 auto 12px', opacity: 0.4 }} />
              ...
              filtered.map((p) => (
                <PacienteRow key={p.id} paciente={p} onEdit={() => openEdit(p)} onDelete={() => onDelete(p.id)} />
              ))
            )}
          </tbody>
        </table>
      ...
      {isOpen && (
        <Modal
          title={editingId !== null ? 'Editar paciente' : 'Novo paciente'}
          onClose={handleClose}
          onSave={handleSave}
        >
